...
# -- RUN THE SERVER ---------------------------------------------------------
# NOTE: every line below logs to stderr, never stdout -- stdout is reserved
# exclusively for MCP's JSON-RPC protocol messages on the stdio transport.

if __name__ == "__main__":
    import sys
    print("Starting DevPulse MCP Server...", file=sys.stderr)
    print(
        "5 tools: search_repositories, get_repo_details, list_open_issues, "
        "list_contributors, get_latest_release",
        file=sys.stderr,
    )
    ...
    if not GITHUB_TOKEN:
        print(
            "NOTE: no GITHUB_TOKEN set -- limited to 60 unauthenticated requests/hour.",
            file=sys.stderr,
        )
